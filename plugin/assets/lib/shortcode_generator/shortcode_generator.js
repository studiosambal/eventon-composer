...
jQuery(document).ready(function($){
	...
	// functionalities s4.6
		$.fn.evo_process_sc = function(sc) {
		    ...
		    // Parse shortcode using regex
		    const regex = /(\w+)(?:\s+([\w_]+)="([^"]*)")*/g;
		    let match = regex.exec(sc);

		    // Set shortcode name
		    if (match && match[1]) {
		        __D.base = __D.sc = match[1];
		    }

		    // Extract attributes
		    sc.replace(/([\w_]+)="([^"]*)"/g, function(_, key, value) {
		        __D.vars[key] = value.replace(/['"]/g, ''); // Clean quotes
		    });
