		...
		private function cal_main_buttons( $A  ){
			...
			if( count( $main_btns ) > 0 ){					
				...
				foreach($main_btns as $ff=>$v){
					...
					echo "<button class='cal_head_btn ".$ff.$add."'>".$v. $add2 . "</button>";	
				}				
			}
					...
					echo "<div class='evo_cal_above evodfx evofx_dr_r marb5 evofx_ww'>";
						...
						foreach($above_head as $ff=>$v){
							...
							if($ff=='evo-gototoday-btn'){
								echo "<button class='cal_head_btn ".$ff."' style='' data-mo='{$focused_month_num}' data-yr='{$focused_year}' data-dy=''>".$v."</button>";
							}else{
								$add =  '';
								...
								echo "<button class='cal_head_btn ".$ff.$add."'>".$v.  "</button>";
							}							
						}
